...
Int_t fwdet_tests(HLoop * loop, const AnaParameters & anapars)
{
  ...
  TCanvas *cResolutionL=new TCanvas("cResolutionL","resolutionL");
  TH1F* vertexXres=new TH1F("vertexXres","Resolution in X cooridinate for #Lambda vertex",400,-200,200);
  TH1F* vertexYres=new TH1F("vertexYres","Resolution in Y cooridinate for #Lambda vertex",400,-200,200);
  TH1F* vertexZres=new TH1F("vertexZres","Resolution in Z cooridinate for #Lambda vertex",400,-200,200);
      ...
      if (pH.size()>=1 && pimH.size()>=1)
	{
	  for (int k=0;k<pH.size();k++){
	    for (int j=0;j<pimH.size();j++){
	      ...
	      //HGeomVector ver_LTg=tool.calcVertexAnalytical(base_Tg,dir_Tg,vertexL,dirL);
	      ...
	      if(pimH[j]->getGeantParentPID()==18 && pH[k]->getGeantParentPID()==18)
		{
		  ...
		  vertexXres->Fill(vertexL.X()-(pimH[j])->getGeantxVertex());
		  vertexYres->Fill(vertexL.Y()-(pimH[j])->getGeantyVertex());
		  vertexZres->Fill(vertexL.Z()-(pimH[j])->getGeantzVertex());
		}
